...
if [ -z "$1" ]; then
  echo "====No TABLE_NAME argument input <./bin/ddb/drop cruddur-messages prod====>"
  exit 1
fi
...
# get all needed metadata from db about users and groups
def get_user_uuids():
  sql = """
    SELECT 
      users.uuid,
...
# create msg group with partition and sort key for ddb
...
* Create table-name `cruddur-messages` with schema-load 

* Check `list-tables` and load `seed` data to ddb, run this scripts

* Create `scan` file for scanning data from ddb
...
> Run script `schema` you should get scanned data from ddb

## Create patterns for conversation

* Create `../ddb/patterns/list-conversation` and `../../get-conversation`
...
* Create `./backend-flask/bin/cognito/list-users` file for automatiс extraction users data
